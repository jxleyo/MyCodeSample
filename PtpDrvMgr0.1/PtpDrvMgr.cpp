...
BOOL GetDeviceOEMDriverFiles(_In_ HDEVINFO Devs, _In_ PSP_DEVINFO_DATA DevInfo)
{
    ...
    if (!FindCurrentDriver(Devs, DevInfo, &driverInfoData)) {
        printf("FindCurrentDriver Failed!\n");
        return FALSE;
    }
...
BOOL update()
{
    ...
    UpdateDriverForPlugAndPlayDevicesProto UpdateFun;
    BOOL reboot = FALSE;
    LPCTSTR hwid = NULL;
    LPCTSTR inf = NULL;
    ...
    printf("LoadLibrary newdev.dll ok！\n");

    UpdateFun = (UpdateDriverForPlugAndPlayDevicesProto)GetProcAddress(newdevMod, UPDATEDRIVERFORPLUGANDPLAYDEVICES);
    if (!UpdateFun)
    {
        printf("Load UpdateFun failed！\n");
        goto final;
    }
    printf("Load UpdateFun ok！\n");

    if (!UpdateFun(NULL, hwid, inf, flags, &reboot)) {
