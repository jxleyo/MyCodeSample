...
VOID RegDebugInt(WCHAR* strValueName,int dNum, PVOID dataValue, ULONG datasizeValue)
{
    ...
    //初始化valueName
    UNICODE_STRING valueName; // 目标字符串
    RtlInitUnicodeString(&valueName, strValueName);
...
NTSTATUS OnDeviceAdd(_In_ WDFDRIVER Driver, _Inout_ PWDFDEVICE_INIT DeviceInit)
{
   ...
   /* ///特殊处理 PNP的查询ID的请求
    ...
    timerConfig.AutomaticSerialization = FALSE;//与ParentObject队列分开排序
...
--*/
{
    ...
    //IoSetActivityIdIrp(pIrp, ActivityId);
...
--*/
{
    ...
    //
   // RegDebug(L"OnInterruptIsr _LogGetActivityId EventWrite_HIDI2C_IO_INTERRUPT_RECEIVED", &activityId, sizeof(activityId));
...
//                memory,
//                0,
//                pInputReportBuffer + 1,//pInputReportBuffer是PUSHORT指针宽度,pInputReportBuffer + 1即是pInputReportBuffer偏移2个字节HID_REPORT_LENGTH_FIELD_SIZE
...
SendMouseFilterReport(PDEVICE_CONTEXT pDevContext, mouse_report_t* pMouseReport)
{
    ...
    //status = WdfRequestReuse(pDevContext->ReuseSend2MouseRequest, &reuseParams);//在完成函数里，函数返回总是成功
    //if (!NT_SUCCESS(status)) {
    ...
    //    IOCTL_INTERNAL_MOUSE_REPORT,//自定义ioctl
    //    pDevContext->RequestBuffer, NULL, pDevContext->RequestBuffer, NULL); //因为参数没变，函数返回总是成功 ，查看MSDN
    ...
    mouseFilterStack->Parameters.DeviceIoControl.IoControlCode = IOCTL_INTERNAL_MOUSE_REPORT;//自定义ioctl
...
NTSTATUS Create_reuse_request(PDEVICE_CONTEXT pDevContext)
{
    ...
    RequestAttributes.ParentObject = MouseDevice; //设置父对象，父对象删除时候，自动删除request
    ...
    WDF_OBJECT_ATTRIBUTES_INIT(&RequestAttributes);
    RequestAttributes.ParentObject = MouseDevice; //设置父对象，父对象删除时候，自动删除
    ...
    status = WdfIoTargetFormatRequestForInternalIoctl(MouseIoTarget, pDevContext->ReuseSend2MouseRequest,
        IOCTL_INTERNAL_MOUSE_REPORT,//自定义ioctl
        pDevContext->RequestBuffer, NULL, pDevContext->RequestBuffer, NULL); //设置IOCTL请求
...
void MouseLikeTouchPad_parse(PDEVICE_CONTEXT pDevContext, PTP_REPORT* pPtpReport, mouse_report_t* pMouseReport)
{
    ...
    if (tp->currentfinger.IsButtonClicked) {//切换中键开关
        tp->Mouse_MButton_mode = !tp->Mouse_MButton_mode;
    }
    ...
    if (tp->Mouse_Pointer_LastIndexNUM == -1 && currentfinger_count > 0) {//鼠标指针、左键、右键、中键都未定义,
        ...
        tp->Mouse_Gesture3Finger_mode = FALSE;//结束3指手势模式
        tp->Mouse_Gesture4Finger_mode = FALSE;//结束4指手势模式
    ...
    else if (tp->Mouse_Pointer_CurrentIndexNUM != -1 && !tp->Mouse_Wheel_mode && !tp->Mouse_Gesture3Finger_mode && !tp->Mouse_Gesture4Finger_mode) {  //指针已定义的非滚轮非手势事件处理
        //查找指针左侧或者右侧是否有并拢的手指作为滚轮模式或者按键模式，当指针左侧/右侧的手指按下时间与指针手指定义时间间隔小于设定阈值时判定为鼠标滚轮/手势否则为鼠标按键，这一规则能有效区别按键与滚轮/手势操作,但鼠标按键和滚轮/手势不能一起使用
        //按键定义后会跟踪坐标所以左键和中键不能滑动食指互相切换需要抬起食指后进行改变，左键/中键/右键按下的情况下不能转变为滚轮/手势模式，
        ...
        for (char i = 0; i < currentfinger_count; i++) {
            ...
            if (abs(distance) > tp->FingerMinDistance && abs(distance) < tp->FingerClosedThresholdDistance && Mouse_Button_Interval < ButtonPointer_Interval_MSEC) {//指针左右侧有并拢的手指按下并且与指针手指起始定义时间间隔小于阈值
                if (currentfinger_count == 2) {//双指滚轮模式
                ...
                else if (currentfinger_count == 3) {//3指手势模式
                     //待实现
                    //tp->Mouse_Gesture3Finger_mode = TRUE;//开启3指手势模式
                   
                }
                else if (currentfinger_count == 4) {//4指手势模式
                    //待实现
                    //tp->Mouse_Gesture4Finger_mode = TRUE;//开启4指手势模式
                    
                }
                break;
            }
            else {
                if (tp->Mouse_MButton_mode) {//鼠标中键开启模式
                    if (tp->Mouse_MButton_CurrentIndexNUM == -1 && abs(distance) > tp->FingerMinDistance && abs(distance) < tp->FingerClosedThresholdDistance && Mouse_Button_Interval > ButtonPointer_Interval_MSEC && dx < 0) {//指针左侧有并拢的手指按下并且与指针手指起始定义时间间隔大于阈值
                        Mouse_MButton_Status = 1; //找到中键
                        tp->Mouse_MButton_CurrentIndexNUM = i;//赋值中键触摸点新索引号
                        continue;  //继续找其他按键，食指已经被中键占用所以原则上左键已经不可用
                    }
                    else if (tp->Mouse_LButton_CurrentIndexNUM == -1 && abs(distance) > tp->FingerClosedThresholdDistance && abs(distance) < tp->FingerMaxDistance && dx < 0) {//指针左侧有手指按下，前面滚轮模式条件判断已经排除了所以不需要考虑与指针手指起始定义时间间隔，
                        Mouse_LButton_Status = 1; //找到左键
                        tp->Mouse_LButton_CurrentIndexNUM = i;//赋值左键触摸点新索引号
                        continue;  //继续找其他按键
                    }
                    else if (tp->Mouse_RButton_CurrentIndexNUM == -1 && abs(distance) > tp->FingerMinDistance && abs(distance) < tp->FingerMaxDistance && dx > 0) {//指针右侧有手指按下，前面滚轮模式条件判断已经排除了所以不需要考虑与指针手指起始定义时间间隔
                        Mouse_RButton_Status = 1; //找到右键
                        tp->Mouse_RButton_CurrentIndexNUM = i;//赋值右键触摸点新索引号
                        continue;  //继续找其他按键
                    }
                    ...
                    if (tp->Mouse_LButton_CurrentIndexNUM == -1 && abs(distance) > tp->FingerMinDistance && abs(distance) < tp->FingerMaxDistance && dx < 0) {//指针左侧有手指按下，前面滚轮模式条件判断已经排除了所以不需要考虑与指针手指起始定义时间间隔，
                        Mouse_LButton_Status = 1; //找到左键
                        tp->Mouse_LButton_CurrentIndexNUM = i;//赋值左键触摸点新索引号
                        continue;  //继续找其他按键
                    }
    ...
    else if (tp->Mouse_Pointer_CurrentIndexNUM != -1 && tp->Mouse_Gesture3Finger_mode) {//3指手势操作模式
    //待实现
    }
    else if (tp->Mouse_Pointer_CurrentIndexNUM != -1 && tp->Mouse_Gesture4Finger_mode) {//4指手势操作模式
    //待实现
    }
    else {
        //其他组合无效
...
void MouseLikeTouchPad_parse_init(PDEVICE_CONTEXT pDevContext)
{
   ...
   tp->Mouse_MButton_mode = TRUE;//默认开启鼠标中键
