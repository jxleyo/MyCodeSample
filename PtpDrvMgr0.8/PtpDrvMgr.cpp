...
TrayDlg:
    ...
    //    MessageBox(NULL, L"CreateWindow Error", L"PtpDrvMgr", MB_OK | MB_ICONSTOP);
    //    return EXIT_FAIL;
    //}
...
BOOL AddTrayIcon(HINSTANCE hInstance)
{
    ...
    wcscpy(TrayIconNID.szTip, _T("PtpDrvMgr Tray"));   
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW + 1);//(HBRUSH)(COLOR_WINDOW+1);//(HBRUSH)GetStockObject(WHITE_BRUSH)
    wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_PTPDRVMGR);// MAKEINTRESOURCEW(IDC_PTPDRVMGR);//NULL
    wcex.lpszClassName  = szWindowClass;//"PtpDrvMgrClassName"
    wcex.hIconSm        = LoadIcon(wcex.hInstance, MAKEINTRESOURCE(IDI_SMALL));
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   HWND hWnd = CreateWindowW(szWindowClass, szTitle, WS_DLGFRAME | WS_MINIMIZEBOX | WS_SYSMENU,//"PtpDrvMgrClassName",""PtpDrvMgrWnd",WS_DLGFRAME | WS_MINIMIZEBOX | WS_SYSMENU, //WS_OVERLAPPEDWINDOW
...
LRESULT CALLBACK TrayWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_USER:
        ...
        case WM_LBUTTONDOWN://左键点击托盘图标
            MessageBox(NULL, L"左键点击托盘", L"PtpDrvMgr", MB_OK);
            ShowWindow(hMainDlgWnd, SW_SHOW);
            UpdateWindow(hMainDlgWnd);
            break;
        case WM_LBUTTONDBLCLK://双击托盘图标
            MessageBox(NULL, L"左键双击托盘", L"PtpDrvMgr", MB_OK);
            break;
        case WM_RBUTTONDOWN://
            {
                ...
                ////添加托盘菜单
                //AppendMenu(hMenuTray, MF_STRING, IDR_SHOW, _T("Show"));
                //AppendMenu(hMenuTray, MF_STRING, IDR_EXIT, _T("Exit"));

                  ////HMENU hSubMenu = GetSubMenu(hMenuTray, 0);

                //EnableMenuItem(pTrayMenu, IDR_PAUSE, MFS_GRAYED);//使某项菜单变灰
                nChoiceCMD = TrackPopupMenu(*pTrayMenu, TPM_RETURNCMD, pt.x, pt.y, NULL, hWnd, NULL);//TPM_RETURNCMD获取选择的菜单项 选项ID//TPM_RIGHTBUTTON
                ...
                MessageBox(NULL, L"右键点击托盘", L"PtpDrvMgr", MB_OK);
                break;
            }
...
BOOL CALLBACK DlgWndProc(HWND hWndDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case WM_INITDIALOG:
        //设置标题栏图标
        ::SendMessage(hWndDlg, WM_SETICON, ICON_BIG, (long)hDlgIcon);//ICON_BIG
        return TRUE;//
        //break;
    case WM_COMMAND:
        {
...
InstSuccess:
    ...
    //启动托盘程序模式
    WinExec("PtpDrvMgr.exe", SW_HIDE);//SW_HIDE//SW_MINIMIZE
}
