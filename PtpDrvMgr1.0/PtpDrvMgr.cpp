...
TrayDlg:
    ...
    {
        hMainDlgWnd = FindWindow( NULL, L"MouseLikeTouchPad_I2C_Driver");//szTitle//需要唯一标识才能有效
        if (hMainDlgWnd) {
            ShowWindow(hMainDlgWnd, nCmdShow);//
            UpdateWindow(hMainDlgWnd);
            ::SetWindowPos(hMainDlgWnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE);//设置窗口置顶
    ...
    SetWindowLong(hDlg, GWL_STYLE, GetWindowLong(hDlg, GWL_STYLE) | DS_SETFOREGROUND | WS_EX_TOOLWINDOW);
    ...
    SetTimer(hTrayWnd, 3, 1500, NULL);//发送气泡消息
...
BOOL AddTrayIcon(HINSTANCE hInstance)
{
    ...
    LPCWSTR TrayClassName = L"PtpDrvMgrTray";
    LPCWSTR TrayWndName = L"PtpDrvMgrWnd";

    WNDCLASSEXW wcex;

    wcex.cbSize = sizeof(WNDCLASSEX);

    wcex.style = CS_HREDRAW | CS_VREDRAW;// CS_HREDRAW | CS_VREDRAW;//NULL
    wcex.lpfnWndProc = TrayWndProc;
    wcex.cbClsExtra = 0;
    wcex.cbWndExtra = 0;
    wcex.hInstance = hInstance;
    wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_PTPDRVMGR));//LoadIcon(NULL,IDI_ERROR)//
    wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);//(HBRUSH)(COLOR_WINDOW+1);//(HBRUSH)GetStockObject(WHITE_BRUSH)
    wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_TRAYMENU);// MAKEINTRESOURCEW(IDC_PTPDRVMGR);//NULL
    wcex.lpszClassName = TrayClassName;//"PtpDrvMgrClassName"
    ...
    hTrayWnd = CreateWindowEx(WS_EX_TOOLWINDOW, TrayClassName, TrayWndName, WS_POPUP, CW_USEDEFAULT, 
    ...
    //SetWindowLong(hTrayWnd, GWL_STYLE, GetWindowLong(hTrayWnd, GWL_STYLE) | WS_EX_TOOLWINDOW);
...
LRESULT CALLBACK TrayWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_USER:
        //if (wParam == pNID->uID) {//IDI_TRAY
            ...
            case WM_LBUTTONDOWN://左键点击托盘图标WM_RBUTTONDOWN//双击托盘图标WM_LBUTTONDBLCLK//
                {
                    GetCursorPos(&pt);//获取鼠标坐标
                    SetForegroundWindow(hWnd);//解决菜单外单击左键菜单不消失的问题
                    ...
                    nChoiceCMD = TrackPopupMenu(hTrackMenu, TPM_LEFTALIGN | TPM_BOTTOMALIGN, pt.x, pt.y, NULL, hWnd, NULL);//TPM_LEFTALIGN | TPM_BOTTOMALIGN | TPM_RETURNCMD//TPM_RETURNCMD获取选择的菜单项 选项ID//TPM_RIGHTBUTTON//把参数中的 TPM_RETURNCMD 去掉， 就会有WM_COMMAND消息
                    //销毁菜单
                    ...
                    if (nChoiceCMD == ID_ABOUT) {//ID_ABOUT//IDC_TRAYMENU
                        ...
                        PostMessage(hMainWnd, WM_COMMAND, IDM_ABOUT, NULL);
                    }
                    ...
                    //    UpdateWindow(hMainDlgWnd);*/
                    //}
                    //if (nChoiceCMD == IDR_EXIT) {//WM_CLOSE
                    //    pNID->hIcon = NULL;
