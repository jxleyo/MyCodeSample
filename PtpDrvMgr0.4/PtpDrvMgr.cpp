...
BOOL GetTouchPad_I2C_hwID()
{
    ...
    char* buffer = new char[filesize + 2];//最后一位为'/0',C-Style字符串的结束符。
    ...
    wcscpy_s(TouchPad_I2C_hwID, (const wchar_t*)buffer);
...
BOOL GetOEMDriverName()
{
    ...
    wcscpy_s(OEMinf_name, (const wchar_t*)buffer);
...
BOOL SaveTouchPad_I2C_hwID()
{
    ...
    LPCWSTR p = TouchPad_I2C_hwID;
    DWORD filesize = 2* wcslen(p);
    DWORD writesize = 0;
    WriteFile(hFile, TouchPad_I2C_hwID, filesize, &writesize, NULL);
    CloseHandle(hFile);//关闭句柄
...
BOOL SaveOEMDriverName()
{
    ...
    LPCWSTR p = OEMinf_name;
    DWORD filesize = 2 * wcslen(p);
    DWORD writesize = 0;
    WriteFile(hFile, OEMinf_name, filesize, &writesize, NULL);
    CloseHandle(hFile);//关闭句柄
