...
static HWND hMainDlgWnd = NULL;

HICON hDlgIcon;//
NOTIFYICONDATA* pNID = NULL;
HMENU* pTrayMenu = NULL;
...
BOOL CALLBACK       DlgWndProc(HWND, UINT, WPARAM, LPARAM);//LRESULT
                     ...
                     _In_ int       nCmdShow)
{
    ...
    if (verInfo.dwMajorVersion < 10) {
        MessageBox(NULL, L"不支持Windows10/11 x64以外的系统版本", L"PtpDrvMgr", MB_OK);
        return EXIT_FAIL;
    }

    if (verInfo.dwBuildNumber < 19041) {//Win10 Build 19041.208(v2004)正式版
        MessageBox(NULL, L"Windows版本低于Win10 Build 19041.208(v2004)正式版，请升级系统后再试。", L"PtpDrvMgr", MB_OK);
        return EXIT_FAIL;
    }
    ...
    if (wcscmp(lpCmdLine, L"Install") == 0) {
        ...
        nCmdShowDlg = SW_SHOWNORMAL;
        goto TrayDlg;
    }
    else if (wcscmp(lpCmdLine, L"Tray") == 0) {//最小化托盘模式
        nCmdShowDlg = SW_HIDE;
        goto TrayDlg;
    }
    else {
...
TrayDlg:
    //只允许程序有一个实例运行
    HANDLE hMutex = CreateMutex(NULL, FALSE, TEXT("OnlyPtpDrvMgrTag"));//第一个参数一定是NULL不然没有用
    if (GetLastError() == ERROR_ALREADY_EXISTS)
    {
        if (!FindWindow(NULL, L"MouseLikeTouchPad_I2C_Driver")) {//szTitle//需要唯一标识才能有效
            CreateDlgWnd();
        }
        else {
            ShowDlgWnd();//显示主对话框窗口
    ...
    if (!AddTrayIcon(hInst)) {
        MessageBox(NULL, L"Create Tray Failed", L"PtpDrvMgr", MB_OK);
        return EXIT_FAIL;
    }

    SetTimer(hMainWnd, 3, 1500, NULL);//发送气泡消息


    if (nCmdShowDlg == SW_SHOWNORMAL) {
        if (!hMainDlgWnd) {
            CreateDlgWnd();
        }
        else {
    ...
    {
        if (hMainDlgWnd) {
            if (!IsDialogMessage(hMainDlgWnd, &msg))
            {
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    hMainWnd = hWnd;
    ShowWindow(hMainWnd, SW_HIDE);//隐藏顶级窗口
    ...
    switch (message)
    {
        case WM_USER: {
            ...
            switch (lParam)
            {
                ...
                case WM_COMMAND: {
                    ...
                    switch (wmId)
                    {
                        case ID_ABOUT: {
                            //显示对话框about标签页
                            SetFocus(hWnd);//获得焦点
                            MessageBox(hWnd, L"ID_ABOUT", L"PtpDrvMgr", MB_OK);
                ...
                case WM_RBUTTONDOWN: {//右键点击托盘图标WM_RBUTTONDOWN
                    //MessageBox(hWnd, L"右键点击托盘", L"PtpDrvMgr", MB_OK);
                    POINT pt;//用于接收鼠标坐标
                    INT nChoiceCMD;//用于接收选择的菜单项ID

                    GetCursorPos(&pt);//获取鼠标坐标
                    SetForegroundWindow(hMainWnd);//解决菜单外单击左键菜单不消失的问题
                    ...
                    //动态创建菜单
                    HMENU hMenuTray = CreatePopupMenu();
                    HMENU hScrMenuTray = LoadMenu(hInst, MAKEINTRESOURCE(IDC_TRAYMENU));//pTrayMenu
                    //添加托盘菜单
                    AppendMenu(hMenuTray, MF_POPUP, (UINT_PTR)hScrMenuTray, _T("Popup"));//附加到弹出菜单
                    //AppendMenu(hMenuTray, MF_STRING, IDR_EXIT, _T("Exit"));
                    //EnableMenuItem(pTrayMenu, IDR_PAUSE, MFS_GRAYED);//使某项菜单变灰

                    HMENU hTrackMenu = GetSubMenu(hMenuTray, 0);//取出目标菜单

                    nChoiceCMD = TrackPopupMenu(hTrackMenu, TPM_LEFTALIGN | TPM_BOTTOMALIGN | TPM_RETURNCMD, pt.x, pt.y, NULL, hWnd, NULL);//TPM_LEFTALIGN | TPM_BOTTOMALIGN | TPM_RETURNCMD//TPM_RETURNCMD获取选择的菜单项 选项ID//TPM_RIGHTBUTTON//把参数中的 TPM_RETURNCMD 去掉， 就会有WM_COMMAND消息
                    //销毁菜单
                    DestroyMenu(hScrMenuTray);
                    DestroyMenu(hMenuTray);

                    if (nChoiceCMD == ID_OPMETHOD) {//ID_ABOUT//IDC_TRAYMENU

                    }

                    if (nChoiceCMD == ID_ABOUT) {//ID_ABOUT//IDC_TRAYMENU
                        PostMessage(hMainWnd, WM_USER, (WPARAM)ID_ABOUT, WM_COMMAND);
                    }
...
BOOL AddTrayIcon(HINSTANCE hInstance)
{
    ...
    NOTIFYICONDATA TrayIconNID;//托盘属性
    pNID = &TrayIconNID;
    HMENU hMenuTray;//托盘菜单
    pTrayMenu = &hMenuTray;

    TrayIconNID.cbSize = sizeof(NOTIFYICONDATA);
    TrayIconNID.hWnd = hMainWnd;//注意父窗口是隐藏的主窗口，和托盘菜单和主对话框窗口独立存在互不干扰
    ...
    wcscpy(TrayIconNID.szInfoTitle, _T(""));//TrayClassName


    //hMenuTray = LoadMenu(hInstance, MAKEINTRESOURCE(IDC_TRAYMENU));
    //hMenuTray = CreatePopupMenu();

    ////添加托盘菜单
    //AppendMenu(hMenuTray, MF_STRING, ID_ABOUT, _T("关于PtpDrvMgr"));


    if (!Shell_NotifyIcon(NIM_ADD, &TrayIconNID)) {
        MessageBox(NULL, L"Shell_NotifyIcon ADD Failed", L"PtpDrvMgr", MB_OK);
        return FALSE;
    }

    //SetWindowLong(hTrayWnd, GWL_STYLE, GetWindowLong(hTrayWnd, GWL_STYLE));
...
void CreateDlgWnd() {
    //创建对话框
    hDlgIcon = LoadIcon(hInst, MAKEINTRESOURCE(IDI_PTPDRVMGR));//MAKEINTRESOURCE(IDI_SMALL)//(LPCWSTR)IDI_SMALL)
    HWND hDlg = CreateDialog(hInst, MAKEINTRESOURCE(IDD_DIALOG_PTPDRVMGR), NULL, (DLGPROC)DlgWndProc);//NULL//hMainWnd//GetDesktopWindow()
    ...
    //PostMessage(hMainDlgWnd, WM_SETICON, ICON_SMALL, (long)hDlgIcon);

    ShowDlgWnd();
}

void ShowDlgWnd() {
   // PostMessage(hMainDlgWnd, WM_SYSCOMMAND, (WPARAM)SC_RESTORE, NULL);
    SwitchToThisWindow(hMainDlgWnd, TRUE);
...
{
    lstrcpy(pNID->szInfoTitle, szTitle);//TrayClassName//szTitle
...
BOOL CALLBACK DlgWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        case WM_INITDIALOG: {
            //MessageBox(hWnd, L"WM_INITDIALOG", L"PtpDrvMgr", MB_OK);
        }
        break;
        
        case WM_ACTIVATE:{
            //MessageBox(hWnd, L"WM_ACTIVATE", L"PtpDrvMgr", MB_OK);
            ShowDlgWnd();
        }
        break;
...
InstSuccess:
    ...
    //存在驱动
    MessageBox(NULL, L"安装驱动成功！请重新启动电脑生效！", L"PtpDrvMgr", MB_OK | MB_DEFBUTTON1);
    ExitCode = EXIT_OK;

    //启动托盘程序模式
    WinExec("PtpDrvMgr.exe Tray", SW_HIDE);//SW_HIDE//SW_MINIMIZE
}
