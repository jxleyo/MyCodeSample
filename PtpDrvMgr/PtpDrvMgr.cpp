...
TrayDlg:
    ...
    SetTimer(hMainDlgWnd, 3, 1500, NULL);//发送气泡消息
...
BOOL AddTrayIcon(HINSTANCE hInstance)
{
    ...
    TrayIconNID.hWnd = hMainDlgWnd;//注意父窗口要是程序主对话框
    ...
    switch (message)
    {
        ...
        case WM_COMMAND:{
            ...
            switch (wmId)
            {
                ...
                case IDM_EXIT: {
                    MessageBox(hWnd, L"IDM_EXIT", L"PtpDrvMgr", MB_OK);
        ...
        case WM_CLOSE:{
            //MessageBox(hWnd, L"WM_CLOSE", L"PtpDrvMgr", MB_OK);
            //不销毁只隐藏窗口以供再次运行程序时显示出来
            ShowWindow(hWnd, SW_HIDE);
            //DestroyWindow(hWnd);
        }
