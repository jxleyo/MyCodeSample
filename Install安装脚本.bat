...
echo检测系统版本
::Win10 Build 19041.208(v2004)正式版 //10.0.a.b格式

ver | find "10.0." > NUL && goto win10
 echo 当前系统不是windows10/11
 exit
...
find/i "oem" infResult.txt && (
    ...
    echo.
     ...
     echo.
     echo 未发现第三方驱动MouseLikeTouchPad_I2C
     echo.
     goto inst
)
...
echo 旧版MouseLikeTouchPad_I2C第三方驱动已经卸载完成
echo.
...
devcon update MouseLikeTouchPad_I2C.inf %hwIDstr%  && (
    echo.
    devcon rescan
    echo MouseLikeTouchPad_I2C第三方驱动已经安装完成
    echo.
    ...
    pause
     ...
     devcon rescan
     echo.
     echo MouseLikeTouchPad_I2C第三方驱动安装准备就绪，按任意键重启电脑后再次运行本安装程序即可成功安装
     pause 
)
echo.
